...
class Classifier :
    def __init__(self,inPath,ground_truth,niter=10,DateTime=None):
        """
        """
        self._inPath = inPath
    ...
    def classify (self):
        ...
        for i in range(self._niter):
            ...
            if not os.path.isfile(os.path.join(self._outPath,"MODELS_%s"%self._datetime,'emp99_model_iteration%s.pkl'%(i+1))):
                ...
                grid = GridSearchCV(rf, param_grid=tuned_parameters, cv=ps, n_jobs=-1)
                ...
                emp99_model = grid.best_estimator_
                emp99_model.fit(emp99_merged_samples,merged_labels)
            ...
            if not os.path.isfile(os.path.join(self._outPath,"MODELS_%s"%self._datetime,'spectral_model_iteration%s.pkl'%(i+1))):
                ...
                spectral_model = grid.best_estimator_
                spectral_model.fit(spectral_merged_samples,merged_labels)
            ...
            if not os.path.isfile(os.path.join(self._outPath,"MODELS_%s"%self._datetime,'emp99+spectral_model_iteration%s.pkl'%(i+1))):
                ...
                total99_model = grid.best_estimator_
                total99_model.fit(total99_merged_samples,merged_labels)
